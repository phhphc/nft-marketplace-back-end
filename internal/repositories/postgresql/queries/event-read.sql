...
AND (e.date >= TO_TIMESTAMP(sqlc.narg('start_date'), 'YYYY-MM-DD') OR sqlc.narg('start_date') IS NULL)
AND (e.date <= TO_TIMESTAMP(sqlc.narg('end_date'), 'YYYY-MM-DD') OR sqlc.narg('end_date') IS NULL);

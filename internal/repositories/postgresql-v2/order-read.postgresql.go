...
func (r *PostgresqlRepository) FindOrder(
	...
	res, err := r.queries.GetOrder(
		ctx,
		gen.GetOrderParams{
			OrderHash: HashToNullString(orderHash),
			Offerer:   AddressToNullString(offerer),

			OfferToken:      AddressToNullString(offer.Token),
			OfferIdentifier: PointerBigIntToNullString(offer.Identifier),

			ConsiderationToken:      AddressToNullString(consideration.Token),
			ConsiderationIdentifier: PointerBigIntToNullString(consideration.Identifier),

			IsCancelled: PointerBoolToNullBool(IsCancelled),
			IsFulfilled: PointerBoolToNullBool(IsFulfilled),
			IsInvalid:   PointerBoolToNullBool(IsInvalid),
		},
	)
	if err != nil {
